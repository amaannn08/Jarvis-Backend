...
export async function initSchema() {
  ...
  try {
    ...
    // 1. Create independent tables first: companies
    await client.query(`
    ...
    `)
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS founder_name TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS meeting_date DATE')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS business_model TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS stage TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS risk_level TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS source_file_name TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS company_domain TEXT')
    await client.query('CREATE INDEX IF NOT EXISTS deals_company_domain_idx ON deals(company_domain)')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS founder_soft_score NUMERIC(4,1)')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS founder_hard_score NUMERIC(4,1)')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS founder_final_score NUMERIC(4,1)')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS dd_recommendation TEXT')
    await client.query('ALTER TABLE deals ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE SET NULL')

    // Now deal_meetings which depends on deals
    await client.query(`
    ...
    `)
    await client.query('CREATE UNIQUE INDEX IF NOT EXISTS deal_meetings_deal_id_idx ON deal_meetings(deal_id)')
    ...
    `)
    await client.query('CREATE INDEX IF NOT EXISTS idx_drive_ingestion_status_status ON drive_transcript_ingestion_status(status)')
    await client.query('CREATE INDEX IF NOT EXISTS idx_drive_ingestion_status_updated_at ON drive_transcript_ingestion_status(updated_at DESC)')
    ...
    `)
    await client.query('CREATE INDEX IF NOT EXISTS idx_deal_identity_ambiguities_status ON deal_identity_ambiguities(status)')
    await client.query('CREATE INDEX IF NOT EXISTS idx_deal_identity_ambiguities_created_at ON deal_identity_ambiguities(created_at DESC)')
    ...
    if (isTimeout) {
      throw new Error('Could not connect to Neon, Check DATABASE_URL')
    }
    throw err
  } finally {
    client.release()
